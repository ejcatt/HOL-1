...
open binderLib
open nomdatatype
val _ = new_theory "oterm";


val tyname = "oterm"

val vp = ``(λn u:unit. n = 0)``
val lp = ``(λn (d:unit + unit + unit) tns uns.
...
val ORC_t = mk_var("ORC", ``:^newty``)
Definition ORC_def:
  ^ORC_t = ^term_ABS_t (GLAM ARB (INR (INR ())) [] [])
End

val ORC_termP = prove(
  ``^termP (GLAM x (INR (INR ())) [] [])``,
  match_mp_tac glam >> srw_tac [][genind_term_REP])

Theorem ORC_def'[local]:
  ^term_ABS_t (GLAM v (INR (INR ())) [] []) = ORC
Proof
  srw_tac [][ORC_def] >> AP_TERM_TAC >> rw[GLAM_NIL_EQ]
QED

val ORC_t = defined_const ORC_def
...
val cons_info =
    [{con_termP = VAR_termP, con_def = VAR_def},
     ...
     {con_termP = ORC_termP, con_def = SYM ORC_def'}]

val tpm_name_pfx = "ot"
val {tpm_thm, term_REP_tpm, t_pmact_t, tpm_t} =
    define_permutation {name_pfx = "ot", name = tyname,
...
val term_ind =
    bvc_genind
        |> INST_TYPE [alpha |-> ``:unit+unit+unit``, beta |-> ``:unit``]
        |> Q.INST [`vp` |-> `^vp`, `lp` |-> `^lp`]
        ...
        |> Q.INST [`Q` |-> `λt. P (oterm_ABS t)`]
        |> SIMP_RULE std_ss [GSYM LAM_def, APP_def', GSYM VAR_def, ORC_def', absrep_id]
...
val LAM_eq_thm = save_thm(
  ...
  ``(LAM u t1 = LAM v t2)``
     |> SIMP_CONV (srw_ss()) [LAM_def, LAM_termP, term_ABS_pseudo11,
                              ...
                              GSYM supp_tpm]
     |> GENL [``u:string``, ``v:string``, ``t1:oterm``, ``t2:oterm``]);
...
val tlf =
  ``λ(v:string) (u:unit + unit + unit) (ds1:(ρ -> α) list) (ds2:(ρ -> α)  list)
                                (ts1:^repty' list) (ts2:^repty' list) (p:ρ).
       if ISR u then if ISL (OUTR u) then tlf (HD ds1) v (oterm_ABS (HD ts1)) p: α 
                     else tof p
       else taf (HD ds2) (HD (TL ds2)) (oterm_ABS (HD ts2))
                (oterm_ABS (HD (TL ts2))) p: α``
...
Theorem FV_otpm[simp] = ``x ∈ FV (otpm p t)``
...
val otpm_eqr = store_thm(
  "otpm_eqr",
  ``(t = otpm pi u) = (otpm (REVERSE pi) t = u)``,
  METIS_TAC [pmact_inverse]);

val otpm_CONS = store_thm(
  "otpm_CONS",
  ``otpm ((x,y)::pi) t = otpm [(x,y)] (otpm pi t)``,
  SRW_TAC [][GSYM pmact_decompose]);

val otpm_ALPHA = store_thm(
  "otpm_ALPHA",
  ``v ∉ FV u ==> (LAM x u = LAM v (otpm [(v,x)] u))``,
  SRW_TAC [boolSimps.CONJ_ss][LAM_eq_thm, pmact_flip_args]);

(* cases theorem *)
val term_CASES = store_thm(
  "term_CASES",
  ``!t. (?s. t = VAR s) ∨ (?t1 t2. t = t1 @@ t2) ∨ (?v t0. t = LAM v t0) ∨ t = ORC``,
  HO_MATCH_MP_TAC simple_induction THEN SRW_TAC [][] THEN METIS_TAC []);

(* should derive automatically *)
Theorem term_distinct[simp]:
  VAR s ≠ APP t1 t2 ∧ VAR s ≠ LAM v t ∧ APP t1 t2 ≠ LAM v t ∧ ORC ≠ VAR s ∧ 
  ORC ≠ APP t1 t2 ∧ ORC ≠ LAM v t
Proof
  srw_tac [][VAR_def, APP_def, LAM_def, LAM_termP, VAR_termP, APP_termP,
             term_ABS_pseudo11, gterm_distinct, GLAM_eq_thm, ORC_def, ORC_termP]
QED
...
val FORALL_TERM = store_thm(
  "FORALL_TERM",
  ``(∀t. P t) <=>
      (∀s. P (VAR s)) ∧ (∀t1 t2. P (t1 @@ t2)) ∧ (∀v t. P (LAM v t)) ∧ (P ORC)``,
...
val otpm_COND = prove(
  ``otpm pi (if P then x else y) = if P then otpm pi x else otpm pi y``,
  SRW_TAC [][]);

val otpm_apart = store_thm(
  "otpm_apart",
  ``!t. ~(x IN FV t) /\ (y IN FV t) ==> ~(otpm [(x,y)] t = t)``,
  metis_tac[supp_apart, pmact_flip_args]);

val otpm_fresh = store_thm(
  "otpm_fresh",
  ``∀t x y. x ∉ FV t ∧ y ∉ FV t ==> (otpm [(x,y)] t = t)``,
  srw_tac [][supp_fresh]);

val rewrite_pairing = prove(
  ``(∃f: oterm -> (string # oterm) -> oterm. P f) <=>
    (∃f: oterm -> string -> oterm -> oterm. P (λM (x,N). f N x M))``,
  EQ_TAC >> strip_tac >| [
...
val subst_exists =
    parameter_tm_recursion
        |> INST_TYPE [alpha |-> ``:oterm``, ``:ρ`` |-> ``:string # oterm``]
        |> SPEC_ALL
        ...
        |> SIMP_RULE (srw_ss()) [support_def, FUN_EQ_THM, fnpm_def,
                                 otpm_COND, otpm_fresh, pmact_sing_inv,
...
val fresh_otpm_subst = store_thm(
  "fresh_otpm_subst",
  ``!t. ~(u IN FV t) ==> (otpm [(u,v)] t = [VAR u/v] t)``,
  HO_MATCH_MP_TAC nc_INDUCTION2 THEN Q.EXISTS_TAC `{u;v}` THEN
  SRW_TAC [][SUB_THM, SUB_VAR]);

val otpm_subst = store_thm(
  "otpm_subst",
  ``!N. otpm pi ([M/v] N) = [otpm pi M/lswapstr pi v] (otpm pi N)``,
...
val otpm_subst_out = store_thm(
  "otpm_subst_out",
  ``[M/v] (otpm pi N) =
       otpm pi ([otpm (REVERSE pi) M/lswapstr (REVERSE pi) v] N)``,
  SRW_TAC [][otpm_subst])
...
val SIMPLE_ALPHA = store_thm(
  ...
  SRW_TAC [][GSYM fresh_otpm_subst] THEN
  SRW_TAC [boolSimps.CONJ_ss][LAM_eq_thm, pmact_flip_args]);
...
Theorem size_otpm[simp] =  GSYM (CONJUNCT2 size_def)
...
val ssub_exists =
    parameter_tm_recursion
        ...
        |> Q.INST [`vr` |-> `\s fm. if s ∈ FDOM fm then fm ' s else VAR s`,
                   ...
                   `A` |-> `{}`]
        |> SIMP_RULE (srw_ss()) [otpm_COND, strterm_fmap_supp, lem2,
...
val otpm_ssub = save_thm("otpm_ssub", CONJUNCT2 ssub_def)
...
val lemma = prove(
  ``(∀x y t. pmact apm [(x,y)] (h t) = h (otpm [(x,y)] t)) <=>
     ∀pi t. pmact apm pi (h t) = h (otpm pi t)``,
  ...
  srw_tac [][Once otpm_CONS] >> srw_tac [][GSYM pmact_decompose]);
...
val term_info_string =
    ...
    \   NTI {nullfv = ``LAM \"\" (VAR \"\")``,\n\
    \        pm_rewrites = [],\n\
    \        pm_constant = ``nomset$mk_pmact oterm$raw_otpm``,\n\
    \        fv_rewrites = [],\n\
    \        recursion_thm = SOME tm_recursion_nosideset,\n\
    \        binders = [(``oterm$LAM``, 0, otpm_ALPHA)]}\n\
    \val _ = type_db :=\n\
    \          Binarymap.insert(!type_db,\n\
    \                           {Name = \"oterm\",Thy=\"oterm\"},\n\
    \                           term_info)\n\
    \in end;\n"
